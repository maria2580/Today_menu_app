...
public class Get_Comments_Thread extends Thread{
    ...
    @Override
    public void run() {
        Data_on_changed.get_instance().setComment_Data(null , null);
        ...
        try {
            Data_on_changed.get_instance().setComment_Data(commentsDto.getContent(), commentsDto.getWritten_time());
        }
        catch (Exception e){
            //System.out.println("이 문구가 나오면 스프링 서버가 작동중이지 않을 가능성이 높습니다.");
            //System.out.println("따라서 댓글이 보이지 않는게 정상입니다.");
            Data_on_changed.get_instance().setComment_Data(new String[1],new String[1]);
        }
        //System.out.println("DTO에 담긴 내용 복사 "+data.getComments_Data());
        try {
            for (int i = 0; i < Data_on_changed.get_instance().getComments_Data().length; i++) {
